...
public class ProductInfo extends JPanel {
    ...
    private void initUI() {
        ...
        // =======================Thêm Panel chứa nút duyệt ảnh và lưu thay
        // đổi==================
    ...
    private void handleButtonAction(String action) {
        switch (action) {
            ...
            case "Sửa":
                ...
                getJtfName().setEditable(true);
                getJcbCategory().setEnabled(true);
                // JOptionPane.showMessageDialog(this, "Bạn có thể sửa tên, loại và hình ảnh món
                // ăn.");
                break;
            case "Xóa":
                ...
                int xacnhanxoa = JOptionPane.showConfirmDialog(null, "Bạn có chắc muốn xóa?", "Xác nhận",
    ...
    public void setEditableAllField() {
        if (getBtChooseImage().isVisible())
            getBtChooseImage().setVisible(false);
        if (getBtUpdateFood().isVisible())
            getBtUpdateFood().setVisible(false);
        if (getBtAddFood().isVisible())
            getBtAddFood().setVisible(false);
        if (getJtfName().isEditable())
            getJtfName().setEditable(false);
        if (getJcbCategory().isEnabled())
            getJcbCategory().setEnabled(false);
    }
